...
- **Responsive Android Portrait Layout & Scaled Typography:** Built with Godot's adaptive `MarginContainer` and `AspectRatioContainer` architecture to scale seamlessly across diverse portrait smartphones (16:9, 18:9, 19.5:9, 20:9) and tablets (4:3, 16:10) without clipping or distortion. Features enlarged 72px letter tiles, doubled center header typography (48px title, 28px mode subtitle, 32px active timer), and 36px toast notifications with comfortable outline padding for effortless readability on mobile displays.
