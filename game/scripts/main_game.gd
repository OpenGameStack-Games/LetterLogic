...
func _ready() -> void:
	...
	if gm != null and gm.game_status != GameManagerScript.GameStatus.IN_PROGRESS:
		...
		if won:
			...
			_show_game_over(titles[idx], msg, true)
		else:
			_show_game_over("Game Over", "The word was %s" % gm.secret_word, false)
